...
export default function WelcomePage({ onBack, onFinish, isDesktop, category }: WelcomePageProps) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: "#fff", animation: "pageIn .35s cubic-bezier(.22,1,.36,1)", overflow: "hidden" }}>
      <div style={{ position: "relative", height: isDesktop ? 240 : 200, flexShrink: 0, background: heroBackground }}>
        {image && (
          <img
            src={image}
      ...
      <div style={{ flexShrink: 0, padding: isDesktop ? "14px 56px 28px" : "10px 22px 20px", background: "white", borderTop: "1px solid #f0f0f0", maxWidth: isDesktop ? 680 : "100%", margin: isDesktop ? "0 auto" : undefined, width: "100%" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ flex: 1, padding: isDesktop ? "14px" : "13px", background: "#ff5a2c", color: "white", border: "none", borderRadius: 14, fontSize: isDesktop ? 15 : 14, fontWeight: 800, cursor: loading ? "default" : "pointer", boxShadow: "0 6px 20px rgba(255,90,44,.35)", opacity: loading ? 0.65 : 1 }}
          >
