// src/pages/Dashboard/Departments.tsx
...
export default function CommonDepartments() {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      toast.error("Failed to fetch common departments");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await api.delete(`/common-departments/${deleteTargetId}`);
      toast.success("Common department deleted successfully");
      fetchDepartments();
    } catch {
      toast.error("Error deleting common department");
    } finally {
  ...
  const openEditForm = (dept: Department) => {
    navigate(`/dashboard/common-departments/${dept.id}/edit`);
  };
  ...
  const filteredData = departments.filter((d) =>
    ...
    (d.details && d.details.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  return (
    <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-700 lg:p-6 dark:bg-gray-900 bg-white relative w-full max-w-[900px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-5 gap-3">
        ...
        <Link
          to="/dashboard/common-departments/add"
        ...
        >
          <Plus size={20} /> Add Common Department
        </Link>
      </div>
        ...
        <table className="min-w-full text-base bg-white dark:bg-gray-900">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {loading ? (
              ...
              paginatedData.map((d) => (
                <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                  ...
                  <td className="px-6 py-4 flex gap-3">
                    <button
                      ...
                      aria-label="Edit common department"
                    >
                      <Edit size={18} />
                    </button>
                    <button
                      ...
                      aria-label="Delete common department"
                    >
                      <Trash2 size={18} />
                    </button>
                  </td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-200 dark:border-gray-700">
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Are you sure you want to delete this common department?{" "}
